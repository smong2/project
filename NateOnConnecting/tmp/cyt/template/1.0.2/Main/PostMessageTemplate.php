<?php /* /home/mong/project/NateOnConnecting/template/Main/PostMessage.tpl */ class Main_PostMessageTemplate extends Podo_Template_Base { const HAVE_CDATA = FALSE; public $myUser; public function printHead($p_intLine, $p_intCol) { ?><script <?php echo '/js/shortcut.js' !== null ? 'src="'.Util::appendFileModifyTime('/js/shortcut.js').'" ' : ''; ?> type="text/javascript"></script>
...
<script <?php echo null !== null ? 'src="'.Util::appendFileModifyTime(null).'" ' : ''; ?> type="text/javascript">
	var msgOpenDefault = <?php echo str_replace('\r', '', json_encode($this->myUser->getMsgOpenDefault())); ?>;
	var isTwitLink = <?php echo str_replace('\r', '', json_encode($this->myUser->isLinkedTwitter())); ?>;
	var isFirstVisit = <?php echo str_replace('\r', '', json_encode($this->myUser->isFirstVisit())); ?>;
	var	firstTxt = "<?php echo htmlspecialchars($this->myUser->getRealname()); ?>님! 지금 머리속을 스치는 생각, 친구들에게 하고싶었던 이야기들을 150자로 커넥팅에 적어보세요";
...
<form id="fo_message" onsubmit="return false;">
	<input type="hidden" id="profileType" name="profileType" value="<?php echo $this->_escapeAttrib($this->myUser->getProfileImage() ? User::PROFILE_TYPE_CUSTOM : 1); ?>" />
	<dl class="cntInput" onmouseover="document.getElementById('aa').style.display='block'" onmouseout="document.getElementById('aa').style.display='none'">
		<dt>
			<a href="javascript:openWindow('http://connect.nate.com/index.nate?menu=18');" onclick="olapStat('CIM03'); openWindow('http://connect.nate.com/index.nate?menu=18'); return false;" target="_blank" onmouseover="document.getElementById('pfManage').style.display='block'" onmouseout="document.getElementById('pfManage').style.display='none'" onfocus="blur();">
				<img src="<?php echo $this->_escapeAttrib($this->myUser->getProfileImage() ? $this->myUser->getProfileImageUrl() : User::generateProfileImageUrl(1)); ?>" width="56" height="56" alt="" />
				<img src="/ui/img/btn_manage.gif" id="pfManage" class="pf_mng" alt="관리" />
			</a>
		</dt>
		<dd class="write"><textarea id="memo" name="memo" cols="50" rows="3" style="overflow:auto;" <?php if ($this->myUser->isFirstVisit()) { ?>onclick="clearFirstTxt()"<?php } ?>></textarea></dd>
		<dd id="aa" class="writeSend" style="display:none;">
			<div class="writeSendbox" id="sendMessage">
				<span class="count">(<span id="len">0</span>/<?php echo htmlspecialchars(Message::MAXLEN_MESSAGE); ?>)</span>
				<div class="sendtype">
					...
					<input type="checkbox" id="keep_use" value="y" checked="checked" style="display:none" /><br />
					<input type="checkbox" id="send_twitter" name="send_twitter" onfocus="this.blur()" onclick="olapStat('CIM05');" disabled="disabled" />
