...
class MessageController extends BaseController
{
	...
	public function doAdd() /*{{{*/
	{
		...
		try {
			...
			} elseif (eregi(ContainerAPI::CHECK_USER_DAYS, $result)) {
				$this->json->result   = ContainerAPI::CHECK_USER_DAYS;
			} elseif (eregi(ContainerAPI::CHECK_USER_FOREVER, $result)) {
				$this->json->result   = ContainerAPI::CHECK_USER_FOREVER;
			}
		} catch (Exception $e) {
	...
	public function doAddComment() /*{{{*/
	{
		...
		try {
			...
			} elseif (eregi(ContainerAPI::CHECK_USER_DAYS, $commentResult)) {
				$this->json->result   = ContainerAPI::CHECK_USER_DAYS;
			} elseif (eregi(ContainerAPI::CHECK_USER_FOREVER, $commentResult )) {
				$this->json->result   = ContainerAPI::CHECK_USER_FOREVER;
			}
		} catch (Exception $e) {
			$this->json->result = false;
	...
	public function doReconnect() /*{{{*/
	{
		...
		try {
			...
			if ($result === true) {
				$this->json->result = true;
			} elseif (strstr($result, ContainerAPI::CHECK_FORBID_WORD.' '.ContainerAPI::FORBID_WORD_TEXT)) {
				$this->json->result   = ContainerAPI::CHECK_FORBID_WORD;
				$this->json->descript = str_replace(ContainerAPI::CHECK_FORBID_WORD.' '.ContainerAPI::FORBID_WORD_TEXT, '', $result);
			} elseif (strstr($result, ContainerAPI::CHECK_USER_DAYS)) {
				$this->json->result   = ContainerAPI::CHECK_USER_DAYS;
			} elseif (strstr($result, ContainerAPI::CHECK_USER_FOREVER)) {
				$this->json->result   = ContainerAPI::CHECK_USER_FOREVER;
			}
		} catch (Exception $e) {
			$this->json->result = false;
