<tpl:Common_Html>
	<tpl:Admin_Head>
		<script type="text/javascript" language="javascript">
			var before = {
				'NickName' : {@$this->myUser->memory->getNickname()}
			}
			...
			<tr><td>
				...
				<tr>
					...
					<td>
						<table>
							<tr>
								<td><img id="img_preview" align="top" src="{$this->myUser->memory->getProfileImageUrl()}" width="80" height="80"/></td>
								<td>
								...
								<input type="hidden" id="pre_profile_img" name="pre_profile_img" value="{$this->myUser->memory->getProfileImage()}">
									<br>
				...
				<tr>
					...
					<td>
						{date('Y년 m월 d일', $this->myUser->memory->getInsertDt())} (오늘은 {$this->firstMemoryDistance} 일째)
					</td>
				</tr>
				<tr>
					<th align="left">* 닉네임</th>
					<td><input type="checkbox" id="nick_fl" name="nick_fl" attr-if:checked="$this->myUser->memory->getNickEnableFl()" value="1" onclick="fnNickCheck()";/>
							<input type="text" id="nick_name" name="nick_name" size="10" value="{$this->myUser->memory->getNickname()}"  onkeydown="return fnEnterCtl()" onchange="$('checkval').value=0" disabled?="!$this->myUser->memory->getNickEnableFl()" />

							<input type="hidden" id="nick_name_pre" name="nick_name_pre" value="{$this->myUser->memory->getNickname()}">

							<input type="button" id="du_confirm" value="중복확인" attr-if:disabled="!$this->myUser->memory->getNickEnableFl()" onclick="checkdup($('nick_name').value)"/>
					...
					<td>
						<table>
							<tr>
								...
								<td><input type="text" id="blog_text" name="blog_text" size="30" value="{$this->myUser->memory->getHomepage()}" onkeydown="return fnEnterCtl()" /><br>
									<input type="checkbox" id="homepage_link_fl" name="homepage_link_fl" value="1" attr-if:checked="$this->myUser->memory->getHomepageLinkFl()" />
									글을 쓸 때 내 공간으로 링크 끊기
								</td>
							</tr>
						</table>
				...
				<tr>
					...
					<td>한 페이지 당
							<select name="per_page">
								<option podo:foreach="array(5, 10, 15, 20, 30) as $intPage=>$pageValue" value="{$pageValue}"
								attr-if:selected="$this->myUser->memory->getViewPerPage()==$pageValue">{$pageValue}개</option>
							</select>
					</td>
				</tr>
