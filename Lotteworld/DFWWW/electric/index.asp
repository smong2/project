...
Dim rs, rsidlist '레코드셋 : 계정확인, 처음방문했는지 확인, 수요예측 계정 리스트 조회
...
IF varinsert="ok" THEN '확인 버튼을 클릭했을 경우
	...
	Set objServer = Nothing
		...
		Response.Write "<script language=javascript>location.replace('admin/admin_acnt_add.asp?VarId=" & VarId & "&VarPwd=" & VarPwd & "')</script>"	'뒤로 가기 스크립트  
		END IF

		IF rs.eof THEN   '로그인시 계정이 없다면 	
			Response.Write "<script language=javascript>alert(' 아이디나 비밀번호가 틀렸습니다. 기억이 나지 않으시면 아래 비밀번호 찾기를 이용해주십시오 ')</script>"
